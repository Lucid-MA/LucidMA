            ...
            with conn.begin():  # Start a transaction
                ...
                update_clause = ", ".join(
                    [
                        ...
                        if col != "bond_data_id"  # Assuming "Factor_ID" is unique and used for conflict resolution
                    ]
                )
...
for filename in os.listdir(directory):
    if filename.startswith(pattern) and filename.endswith(".xlsx") and filename not in read_processed_files():
        ...
        df = df[["cusip", "security_typ", "issuer", "collat typ", "name", "mtg factor", "issue dt", "maturity",
                 "amt outstanding", "coupon", "rtg_sp", "rtg_moody", "rtg_fitch", "rtg_kbra", "rtg_dbrs",
                 "rtg_egan_jones"]]
        ...
        df.rename(columns={
            ...
            inplace=True)
        # Create Price_ID
        df["bond_data_id"] = df.apply(lambda row: hash_string(f"{row['bond_id']}{filename}"), axis=1)
        df["bond_data_date"] = date
        df['bond_data_date'] = pd.to_datetime(df['bond_data_date'], format='%Y-%d-%m').dt.strftime('%Y-%m-%d')
        df['is_am'] = is_am
        df["source"] = filename
