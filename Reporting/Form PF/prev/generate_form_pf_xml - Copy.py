...
import win32com.client as win32 # might need pywin32 version 224... pip install --upgrade pywin32==224 and clear out the gen_py folder
from lxml import etree
from datetime import datetime

# PARAMETERS TO INITIALIZE
FILING_DATE = "2022-09-30" # quarter-end here as YYYY-MM-DD string
DOING_MMT = False
WORKBOOK_PATH = "S:\\Mandates\\Funds\\Fund Reporting\\Form PF working files\\curr\\q63_book.xlsx"
XML_OUTPUT_PATH = "lucid_form_pf_" + datetime.now().strftime("%Y%m%d_%H_%M_%S") + ".xml"
...
SECTION_1B_SHEETS = ["Section 1b - Priv Fnd USG M", "Section 1b - Prv Fnd Prime M",
                     "Section 1b - Prv Fnd Prime C1", "Section 1b - Prv Fnd Prime Q1",
                     'Section 1b - Prv Fnd Prime MIG', "Section 1b - Prv Fnd Prime Q364",
                     "Section 1b - Prv Fnd Prime QX", "Section 1b - Prv Fnd Prime S1"]

if DOING_MMT:
    SECTION_1B_SHEETS.append("Section 1b - Prv Fnd MMT T")

SECTION_3_SHEETS1 = ["Sec 3 Item A-C USG M",     "Sec 3 Item A-C Prime M",
                     "Sec 3 Item A-C Prime C1",  "Sec 3 Item A-C Prime Q1",
                     "Sec 3 Item A-C Prime MIG", "Sec 3 Item A-C Prime Q364",
                     "Sec 3 Item A-C Prime QX", "Sec 3 Item A-C Prime S1"]
SECTION_3_SHEETS2 = ["Sec 3 Item D-E USG M",     "Sec 3 Item D-E Prime M",
                     "Sec 3 Item D-E Prime C1",  "Sec 3 Item D-E Prime Q1",
                     "Sec 3 Item D-E Prime MIG", "Sec 3 Item D-E Prime Q364",
                     "Sec 3 Item D-E Prime QX", "Sec 3 Item D-E Prime S1"]

# ORDER MATTERS HERE - Q63_PATHS AND FUND_DATA must be parallel (change line) in section 3E
Q63_PATHS = ["S:\\Mandates\\Funds\\Fund Reporting\\Form PF working Files\\curr\\q63\\2022_7_8_9_Prime_Custom1.xlsx",
             "S:\\Mandates\\Funds\\Fund Reporting\\Form PF working Files\\curr\\q63\\2022_7_8_9_Prime_Monthly.xlsx",
             "S:\\Mandates\\Funds\\Fund Reporting\\Form PF working Files\\curr\\q63\\2022_7_8_9_Prime_MonthlyIG.xlsx",
             "S:\\Mandates\\Funds\\Fund Reporting\\Form PF working Files\\curr\\q63\\2022_7_8_9_Prime_Quarterly1.xlsx",
             "S:\\Mandates\\Funds\\Fund Reporting\\Form PF working Files\\curr\\q63\\2022_7_8_9_Prime_QuarterlyX.xlsx",
             "S:\\Mandates\\Funds\\Fund Reporting\\Form PF working Files\\curr\\q63\\2022_7_8_9_Prime_Q364.xlsx",
             "S:\\Mandates\\Funds\\Fund Reporting\\Form PF working Files\\curr\\q63\\2022_7_8_9_Prime_S1.xlsx",
             "S:\\Mandates\\Funds\\Fund Reporting\\Form PF working Files\\curr\\q63\\2022_7_8_9_USG_Monthly.xlsx"]


FUND_DATA = [["Lucid Prime Fund LLC [Series C1]", "805-3531452546", "LIQUIDITY"],
             ...
             ["Lucid Prime Fund LLC [Series S1]", "805-6436074477", "LIQUIDITY"],
...
def main():
    ...
    print("Validating XML output...") # one of the JJV additions - validate the xml output against the xsd schema
    try:
...
def section1a_itemB(wb):
    sheet = wb.Worksheets("Items B and C")
...
def section1c_itemB(wb):
    ...
    for x in range(1,6):
        ...
        ET.SubElement(cps, "CounterpartyAffiliatedInstitutionCode").text =  "31" if (sheet.Range("C"+str(x+73)).Value == "NA" or sheet.Range("C"+str(x+73)).Value == "N/A") else "27"
        ...
        while sheet.Range("A" + str(index)).Value is not None:
            ...
            if tmp_cusip == NA:
                ...
                if tmp_cusip == 'X9USDDGCM':
                    tmp_cusip == 262006208
                try:
                    ET.SubElement(security, "CUSIP").text = str(int(float(tmp_cusip))) # if all numeric cusip
                    if len(str(int(float(tmp_cusip)))) != 9:
                        print("WARNING: CUSIP wrong length: " + str(int(float(tmp_cusip))))
                except ValueError:
            ...
            if sheet.Range("J" + str(index)).Value != NA:
                ...
                if tmp_cusip == NA:
                    ...
                    try:
                        ET.SubElement(repo, "CUSIP").text = str(int(float(tmp_cusip))) # if all numeric cusip
                    except ValueError:
                ...
                if tmp_maty == NA or tmp_maty is None:
                    ...
                    try:
                        if float(tmp_maty) <= 1.0: # if perpetual
                             ...
                             "NOTICE: Perpetual bond detected. " + tmp_cusip + " Make note on comments page."
                    except ValueError:
