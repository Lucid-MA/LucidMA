...
def create_transactions_table(tb_name):
    ...
    try:
        columns_sql = ', '.join(
            [f'"{col}" TEXT' for col in needed_columns] + ['"FileDate" DATE', '"TransactionID" TEXT', '"FileName" TEXT'])
...
def validate_schema_and_update_db(excel_dir, tb_name):
    ...
    for file in os.listdir(excel_dir):
        if file.endswith('.xlsx'):
            ...
            if not file_date:
                print(f"Skipping {file} as it does not contain a correct date format in file name.")
                continue

            # Read Excel data and add FileName, FileDate columns
            # Read Excel data
            df_header = pd.read_excel(file_path, nrows=0)  # Reads only the header
            missing_columns = [col for col in needed_columns if col not in df_header.columns]
            if missing_columns:
            ...
            print(f"Data from {file} successfully upserted into {tb_name}. Processing time: {process_time:.2f} seconds")


# Main execution
TABLE_NAME = 'transactions_raw_v2'
excel_dir = r'S:\Users\THoang\Data\SSC'
needed_columns = ['SK', 'PoolCode', 'PoolDescription', 'Period', 'PeriodDescription', 'InvestorCode', 'InvestorDescription', 'Head1', 'Amt1']
