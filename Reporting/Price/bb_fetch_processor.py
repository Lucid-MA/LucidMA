...
# added some col 
fields = "SECURITY_TYP,ISSUER,Collat Typ,Name,Industry Sector,Issue DT,Maturity,Amt Outstanding,Coupon,Floater," \
         ...
         "TRADE_DT_ACC_INT,PRINCIPAL_FACTOR,MTG_PREV_FACTOR,MTG_RECORD_DT,MTG_FACTOR_PAY_DT,MTG_NXT_PAY_DT_SET_DT,IDX_RATIO" # 6/24/2020 added principal factor, 9/18/2020 added more factor data

cols = "CUSIP,SECURITY_TYP,ISSUER,Collat Typ,Name,Industry Sector,Issue DT,Maturity,Amt Outstanding,Coupon,Floater," \
...
def fetch_helix_symbols():
    ...
    for f in ["Prime", "USG", "MMT", "LMCP Inv"]:
        helix_backup_file = PureWindowsPath(Path("S:/Mandates/Operations/Helix Trade Files/" + f + (" Fund" if f != "LMCP Inv" else "") + ".txt"))
        fund_df = pd.read_csv(helix_backup_file, sep="\t")
        for c in fund_df["BondID"]:
            if isinstance(c, str) and c.strip().isalnum():
                    cusips_all[c] = (fund_df[fund_df['BondID'] == c]['Description'].iloc[0]).strip()
    return cusips_all

# to translate from helix symbol to the associated code
symbol_classifiers = {'Treasury Bill': 'TBILL', 'Treasury Note': 'TNOTE', 'Treasury Bond': 'TBOND', 'Agency NOTE': 'AGNCYNT', 'Agency MBS FIX PT': 'MBSFIX', 
'Agency MBS ARM PT': 'MBSARM', 'Agency MBS TRUST': 'MBSTRUST', 'Agency MBS CMO': 'MBSCMO', 'Agency MBS IO FIX': 'MBSIOFIX', 'Agency MBS IO INV': 'MBSIOINV', 
'Agency MBS PO': 'MBSPO', 'Agency COMMERCIAL PT': 'CMRCLPT', 'Agency COMMERCIAL IO': 'CMRCLIO', 'Agency PROJECT LOAN PT': 'PROJECTPT', 
'Agency PROJECT LOAN IO': 'PROJECTIO', 'GNMA MBS FIX PT': 'MBSFIXGMNA', 'GNMA MBS ARM PT': 'MBSARMGMNA', 'GNMA MBS TRUST': 'MBSTRUSTGMNA', 'GNMA MBS CMO': 'MBSCMOGMNA', 
'GNMA MBS IO FIX': 'MBSIOFIXGMNA', 'GNMA MBS IO INV': 'MBSIOINVGMNA', 'GNMA MBS PO': 'MBSPOGMNA', 'GNMA COMMERCIAL PT': 'CMRCLPTGMNA', 'GNMA COMMERCIAL IO': 'CMRCLIOGMNA', 
'GNMA PROJECT LOAN PT': 'PROJECTPTGMNA', 'GNMA PROJECT LOAN IO': 'PROJECTIOGMNA', 'SBA SBA REAL ESTATE': 'SBARE', 'SBA SBA EQUIP': 'SBAEQUIP', 'CLO BSL': 'CLOBSL', 
'CLO MM': 'CLOMM', 'CLO CRE': 'CLOCRE', 'CLO OTHER': 'CLOOTHER', 'RMBS CRT': 'RMBSCRT', 'RMBS LEGACY': 'RMBSLGCY', 'RMBS RECENT': 'RMBSRECNT', 'RMBS RPL': 'RMBSRPL', 
'RMBS QM': 'RMBSQM', 'RMBS NON QM': 'RMBSNONQM', 'RMBS OTHER': 'RMBSOTHER', 'CMBS CONDUIT': 'CMBSCOND', 'CMBS NON-AGNC': 'CMBSNA', 'CMBS SINGLE PROPERTY': 'CMBSSINGLE', 
'CMBS OTHER': 'CMBSOTHER', 'ABS AUTO': 'ABSAUTOS', 'ABS CREDIT CARD': 'ABSCC', 'ABS EQUIPMENT': 'ABSEQUIP', 'ABS AIRCRAFT': 'ABSAIRCFT', 'ABS OTHER': 'ABSOTHER', 
'CDO OTHER': 'CDOOTHER', 'CDO TRUPS': 'CDOTRUPS', 'CDO BONDS': 'CDOBONDS', 'EURO ABS OTHER': 'EUROABSOTHER', 'FINANCIAL SENIOR FLOAT': 'FINSENFLT', 
'FINANCIAL SENIOR FIX': 'FINSENFIX', 'FINANCIAL SUB FLOAT': 'FINSUBFLT', 'FINANCIAL SUB FIX': 'FINSUBFIX', 'CORPORATE 1ST LIEN FLOAT': 'CORP1LFLT', 
'CORPORATE 1ST LIEN FIX': 'CORP1LFIX', 'CORPORATE 2ND LIEN FLOAT': 'CORP2LFLT', 'CORPORATE 2ND LIEN FIX': 'CORP2LFIX', 'CORPORATE SENIOR FLOAT': 'CORPSENFLT', 
'CORPORATE SENIOR FIX': 'CORPSENFIX', 'CORPORATE SUB FLOAT': 'CORPSUBFLT', 'CORPORATE SUB FIX': 'CORPSUBFIX', 'CD': 'MMCD', 'CP': 'MMFCP', 'ABS CP': 'MMABSCP', 
'NON FIN CP': 'MMNFCP', 'VRDN': 'MMMUNI', 'MUNI GO FIX': 'MUNIGOFIX', 'MUNI GO FLT': 'MUNIGOFLT', 'MUNI REV FIX': 'MUNIREVFIX', 'MUNI REV FLT': 'MUNIREVFLT', 
'MUNI ABS': 'MUNIABS', 'CMBS CONDUIT IO FIX': 'CMBSCONDIO', 'CMBS SMALL BALANCE': 'CMBSSBC', 'ABS STUDENT LOAN': 'ABSSTLN', 'ABS WHOLE BUSINESS': 'ABSWBS', 
'ABS CONSUMERS': 'ABSCON', 'ABS TIMESHARE': 'ABSTSHR', 'ABS SMALL BUS LOAN': 'ABSSBLN', 'TRI PARTY IG CORP': 'TRIPTYIGCORP', 'RMBS SFR': 'RMBSSFR', 'RMBS PRIVATE CRT': 'RMBSCRTPVT',
'RMBS IO FIX': 'RMBSIO', 'RMBS IO INV': 'RMBSIOINV','RMBS MTG SVC RIGHTS': 'RMBSMSR', 'RMBS MTG INSURANCE': 'RMBSMI', 'RMBS MANUF HOUSING': 'RMBSMNF', 'GERMAN BUND': 'GEBUND', 'FRENCH OAT': 'FROAT',
'USD Cash': 'CASH', 'French Govt Bond': 'FROAT'}

# these are cusips in BBerg but different ticker to access
diff_cusip_map = {'EURFX':'EUR Curncy', 'XS2606220999':'PPG91FR41', 'XS2592024009':'PPGA0OKN5', 'XS2592025071':'PPG80PFP8', 'ALMNDUSD7': 'PPFM1GA98', 'ALMNDUSD8': 'PPFR5TTD6', 'STHAPPLE5': 'PPFT6V9U0','OPPORTUN1': 'PPFQKMXT6', 'STHAPPLE4': 'PPF54YY06','MNTNCHRY1': 'PPEBEGFI4', 'MNTNCHRY2': 'PPFX3C1P5','52953BBJ1': 'PPEG3JY56', 'STHAPPLE2': 'PPED2BZX9', 'XS2373029664':'PPEZDK875', '52468JX82':'PPE43E6K2', 'XS2373029748':'PPE0FKE65', 'STHAPPLE1': 'PPE32P4N6', 'HEXZETA01': 'PPE9DMNR8','HEXZETA02': 'PPE4DGC45','STHAPPLE3': 'PPE939O30','ALMNDUSD4': 'PPE0F22A9', 'ALMNDUSD5': 'PPEA34F46', 'ALMNDUSD6': 'PPEBFPHO8', 'ALMNDEUR4': 'PPE5GG8O0', 'ALMNDEUR3': 'PPEA34F53','ALMNDUSD3': 'PPE139546', 'ALMNDUSD2': 'PPEE2UU10', 'ALMNDUSD1': 'PPE32P4O4', 'ALMNDEUR2': 'PPE42XYF1', 'ALMNDEUR1': 'PPEA2SM53', 'XS2225938831': 'PPEF1MMY3', 'XS2091648928': 'PP9FCKDZ8', 'XS1951177309': 'PP30JD700', 'XS2004377136': 'PP075HWJ5', 'XS2643730695':'PPG64I278', 'XS2644211281':'PPG24HYG4', 'XS2644210986':'PPG64I468', 'XS2644211109':'PPG64I4G6','TREATYUS1':'PPG1JQ3D1','XS2643730695':'PPG1K4CZ9','XS2644210986':'PPG5K61F1','XS2644211109':'PPG1K4CY2','XS2644211281':'PPG5K61D6'}
...
def bb_fetch(cusips):
    ...
    df['CUSIP'] = cusips
    cusip_pass = [(diff_cusip_map[x] if x in diff_cusip_map.keys() else x) for x in cusips]
...
def bb_fetch_with_overrides(mktsymbol_map):
    ...
    param_file = PureWindowsPath(Path("S:/Lucid/Data/Bond Data/Bloomberg_Calc_Parameters.xlsx"))
    ...
    for i, row in distincts.iterrows():
        ...
        for col in cols:
            ...
            overrides = overrides + " " + (override_name_map[col] if col in override_name_map else col) + " " + str(row[col])
        ...
        cmd = "java -cp \"blpapi3.jar;\" BloombergFetch \"" + list_path + "\" \"" + fields + "\"" + overrides
...
def lucid_rating(sp, moodys, fitch, kroll, dbrs, ej, issuer, sectype):
    ...
    if sectype[:9] == 'Agncy CMO' or sectype[:10] == 'Agncy CMBS':
        return 'USGCMO'
    elif sectype[:10] == 'US GOVERNM' or issuer[:10] == 'Fannie Mae' or issuer[:11] == 'Freddie Mac' or issuer[:28] == 'Government National Mortgage' or sectype[:3] == 'SBA':
        if not (issuer[-5:] == "STACR" or issuer[-3:] == "CRT" or issuer == "Fannie Mae - CAS"): # check if CRT
            return 'USG'
    ...
    rmap = lambda x: 0 if x[:3] == 'AAA' else 1 if x[:2] == 'AA' else 2 if x[:1] == 'A' else 3 if (x[:3] == 'BBB' or x[:3] == 'BAA') else 4 if (x[:2] == 'BB' or x[:2] == 'BA') else 5

    ratingsindex = [rmap(x.upper()) for x in [sp[3:] if sp[:3] == '(P)' else sp, moodys, fitch, kroll, dbrs, ej]]
...
def helix_mkt_symbol(sectype, issuer, colltype, issuedt, bondname, indsector, tranchetype, floater, legacydt):
    ...
    if tobeset and sectype[:5] == "Agncy":
        tobeset = False
        if "CMBS" in sectype: isCommercial = True
        if sectype[-2:] == "IO" or "IO" in tranchetype:
            if "INV" in tranchetype:
                isInvIo = True
            else:
                isFixIO = True
        if sectype[-2:] == "PO": isPO = True
        outp = (("CMRCLIO" if isFixIO else "CMRCLIOINVFIX" if isInvIo else "CMRCLPO" if isPO else "CMRCLPT") if isCommercial else
                ("MBSIOFIX" if isFixIO else "MBSIOINV" if isInvIo else "MBSPO" if isPO else "MBSCMO")) + gnmasuffix
    ...
    if tobeset and colltype == 'US GOVT':
        tobeset = False
        outp = 'TBill' if bondname == 'TREASURY BILL' else 'TNote' if bondname == 'US TREASURY N/B' else 'Unknown!'
    # if clo
    if tobeset and colltype[:6] == 'CF-CLO':
        tobeset = False
        outp = 'CLOBSL' if colltype[-2:] == "LL" else 'CLOMM' if colltype[-3:] == 'MML' else 'Unknown!'
        # if muni
    ...
    if tobeset and indsector == 'Financial':
        tobeset = False
        outp = 'FINSUB' + fixsuffix if colltype == 'SUBORDINATED' else 'FINSEN' + fixsuffix if colltype == 'SR UNSECURED' else 'Unknown!'
    # if rmbs
...
def fetch_spec_df():
    outp = pd.DataFrame([
        {"CUSIP": "CASHUSD01", "SECURITY_TYP": "CASH", "ISSUER": "US TREASURY", "Collat Typ": "Cash", "Name": "Cash",
         "Industry Sector": "USD Cash", "Issue DT": "7/4/1776", "Maturity": "12/31/2099", "Amt Outstanding": "1",
         "Coupon": "0", "Floater": "N", "MTG Factor": "1", "PX Bid": "100", "PX Mid": "100", "Int Acc": "0", "Mtg WAL": "0",
         "DUR ADJ OAS BID": "0", "YAS_MOD_DUR": "0", "USED DURATION": "0.00273972", "Days Acc": "0", "YLD_ytm_BID": "0",
         "I_SPRD_BID": "0", "FLT_SPREAD": "", "OAS_SPREAD_ASK": "0", "MTG TRANCHE TYP LONG": "CASH",
         "MTG PL CPR 1M": "0", "MTG PL CPR 6M": "0", "MTG_WHLN_GEO1": "0", "MTG_WHLN_GEO2": "0", "MTG_WHLN_GEO3": "0",
         "RATINGS BUCKET": "USG", "RTG_SP": "", "RTG_MOODY": "", "RTG_FITCH": "", "RTG_KBRA": "", "RTG_DBRS": "",
         "RTG_EGAN_JONES": "", "DELIVERY_TYP": "", "Est'd Asset Class": "CASH", "CUSIP or ISIN": " CUSIP",'MTG_PREV_FACTOR': '1', 'MTG_RECORD_DT': '', 'MTG_FACTOR_PAY_DT': '', 
         'MTG_NXT_PAY_DT_SET_DT': ''},
	{"CUSIP": "JPCASHUSD", "SECURITY_TYP": "CASH", "ISSUER": "US TREASURY", "Collat Typ": "Cash", "Name": "Cash",
         "Industry Sector": "USD Cash", "Issue DT": "7/4/1776", "Maturity": "12/31/2099", "Amt Outstanding": "1",
         "Coupon": "0", "Floater": "N", "MTG Factor": "1", "PX Bid": "100", "PX Mid": "100", "Int Acc": "0", "Mtg WAL": "0",
         "DUR ADJ OAS BID": "0", "YAS_MOD_DUR": "0", "USED DURATION": "0.00273972", "Days Acc": "0", "YLD_ytm_BID": "0",
         "I_SPRD_BID": "0", "FLT_SPREAD": "", "OAS_SPREAD_ASK": "0", "MTG TRANCHE TYP LONG": "CASH",
         ...
         "RTG_EGAN_JONES": "", "DELIVERY_TYP": "", "Est'd Asset Class": "CASH", "CUSIP or ISIN": " CUSIP",'MTG_PREV_FACTOR': '1', 'MTG_RECORD_DT': '', 'MTG_FACTOR_PAY_DT': '', 
         'MTG_NXT_PAY_DT_SET_DT': ''},
         {"CUSIP": "CASHEUR01", "SECURITY_TYP": "CASH", "ISSUER": "ECB", "Collat Typ": "Cash", "Name": "Cash",
         "Industry Sector": "EUR Cash", "Issue DT": "1/1/1999", "Maturity": "12/31/2099", "Amt Outstanding": "1",
         "Coupon": "0", "Floater": "N", "MTG Factor": "1", "PX Bid": "100", "PX Mid": "100", "Int Acc": "0", "Mtg WAL": "0",
         "DUR ADJ OAS BID": "0", "YAS_MOD_DUR": "0", "USED DURATION": "0.00273972", "Days Acc": "0", "YLD_ytm_BID": "0",
         "I_SPRD_BID": "0", "FLT_SPREAD": "", "OAS_SPREAD_ASK": "0", "MTG TRANCHE TYP LONG": "CASH",
         "MTG PL CPR 1M": "0", "MTG PL CPR 6M": "0", "MTG_WHLN_GEO1": "0", "MTG_WHLN_GEO2": "0", "MTG_WHLN_GEO3": "0",
         ...
         "RTG_EGAN_JONES": "", "DELIVERY_TYP": "", "Est'd Asset Class": "CASH", "CUSIP or ISIN": " CUSIP",'MTG_PREV_FACTOR': '1', 'MTG_RECORD_DT': '', 'MTG_FACTOR_PAY_DT': '', 
         'MTG_NXT_PAY_DT_SET_DT': ''},
        {'CUSIP': '74039RAA8', 'SECURITY_TYP': '', 'ISSUER': 'Preeti Trust SWMC 2021', 'Collat Typ': '', 'Name': '', 
        'Industry Sector': '', 'Issue DT': '6/1/2021', 'Maturity': '12/31/2021', 'Amt Outstanding': '50000000', 'Coupon': '2.96', 
        'Floater': '', 'MTG Factor': '1', 'PX Bid': '101.5', 'PX Mid': '101.5', 'Int Acc': '0', 'Mtg WAL': '5.2', 
        ...
        'MTG_WHLN_GEO1': '', 'MTG_WHLN_GEO2': '', 'MTG_WHLN_GEO3': '', 'RATINGS BUCKET': 'USG', 'RTG_SP': '', 'RTG_MOODY': '', 
        'RTG_FITCH': '', 'RTG_KBRA': '', 'RTG_DBRS': '', 'RTG_Egan_Jones': '', 'DELIVERY_TYP': 'PHYS', "Est'd Asset Class": 'MBSTRUST', 
        'CUSIP or ISIN': ' CUSIP', 'MTG_PREV_FACTOR': '', 'MTG_RECORD_DT': '', 'MTG_FACTOR_PAY_DT': '', 'MTG_NXT_PAY_DT_SET_DT': ''},
        {'CUSIP': '371494AK1', 'SECURITY_TYP': '', 'ISSUER': 'Tri Party IGCorp', 'Collat Typ': '', 'Name': 'Tri Party IGCorp', 
        'Industry Sector': '', 'Issue DT': '8/10/2021', 'Maturity': '8/10/2031', 'Amt Outstanding': '10000000', 'Coupon': '0', 
        'Floater': 'N', 'MTG Factor': '1', 'PX Bid': '100', 'PX Mid': '100', 'Int Acc': '0', 'Mtg WAL': '5', 
        'DUR ADJ OAS BID': '5', 'YAS_MOD_DUR': '5', 'USED DURATION': '5', 'Days Acc': '', 'YLD_ytm_BID': '', 'I_SPRD_BID': '', 
        'FLT_SPREAD': '', 'OAS_SPREAD_ASK': '', 'MTG TRANCHE TYP LONG': '', 'MTG PL CPR 1M': '', 'MTG PL CPR 6M': '', 
        'MTG_WHLN_GEO1': '', 'MTG_WHLN_GEO2': '', 'MTG_WHLN_GEO3': '', 'RATINGS BUCKET': 'BBB', 'RTG_SP': '', 'RTG_MOODY': '', 
        'RTG_FITCH': '', 'RTG_KBRA': '', 'RTG_DBRS': '', 'RTG_Egan_Jones': '', 'DELIVERY_TYP': 'PHYS', "Est'd Asset Class": 'TRIPTYIGCORP', 
        'CUSIP or ISIN': ' CUSIP', 'MTG_PREV_FACTOR': '', 'MTG_RECORD_DT': '', 'MTG_FACTOR_PAY_DT': '', 'MTG_NXT_PAY_DT_SET_DT': ''},
        {'CUSIP': '2063C0VL3', 'SECURITY_TYP': '', 'ISSUER': 'Concord', 'Collat Typ': '', 'Name': 'Concord', 
        ...
        'DUR ADJ OAS BID': '', 'YAS_MOD_DUR': '', 'USED DURATION': '', 'Days Acc': '', 'YLD_ytm_BID': '', 'I_SPRD_BID': '', 
        'FLT_SPREAD': '', 'OAS_SPREAD_ASK': '', 'MTG TRANCHE TYP LONG': '', 'MTG PL CPR 1M': '', 'MTG PL CPR 6M': '', 
        'MTG_WHLN_GEO1': '', 'MTG_WHLN_GEO2': '', 'MTG_WHLN_GEO3': '', 'RATINGS BUCKET': 'A1/P1', 'RTG_SP': '', 'RTG_MOODY': '', 
        'RTG_FITCH': '', 'RTG_KBRA': '', 'RTG_DBRS': '', 'RTG_Egan_Jones': '', 'DELIVERY_TYP': 'PHYS', "Est'd Asset Class": 'MMFCP', 
        'CUSIP or ISIN': ' CUSIP', 'MTG_PREV_FACTOR': '', 'MTG_RECORD_DT': '', 'MTG_FACTOR_PAY_DT': '', 'MTG_NXT_PAY_DT_SET_DT': ''},
        {'CUSIP': 'JPM-TH2O', 'SECURITY_TYP': '', 'ISSUER': 'Concord', 'Collat Typ': '', 'Name': 'JPM-TH2O', 
        'Industry Sector': '', 'Issue DT': '8/13/2021', 'Maturity': '8/20/2021', 'Amt Outstanding': '10000000', 'Coupon': '0', 
        'Floater': 'N', 'MTG Factor': '1', 'PX Bid': '100', 'PX Mid': '100', 'Int Acc': '0', 'Mtg WAL': '0', 
        'DUR ADJ OAS BID': '', 'YAS_MOD_DUR': '', 'USED DURATION': '', 'Days Acc': '', 'YLD_ytm_BID': '', 'I_SPRD_BID': '', 
        'FLT_SPREAD': '', 'OAS_SPREAD_ASK': '', 'MTG TRANCHE TYP LONG': '', 'MTG PL CPR 1M': '', 'MTG PL CPR 6M': '', 
        ...
        'CUSIP or ISIN': ' CUSIP', 'MTG_PREV_FACTOR': '', 'MTG_RECORD_DT': '', 'MTG_FACTOR_PAY_DT': '', 'MTG_NXT_PAY_DT_SET_DT': ''},
        {'CUSIP': 'JPM-SCHF1', 'SECURITY_TYP': '', 'ISSUER': 'Concord', 'Collat Typ': '', 'Name': 'JPM-SCHF1', 
        'Industry Sector': '', 'Issue DT': '8/13/2021', 'Maturity': '8/20/2021', 'Amt Outstanding': '10000000', 'Coupon': '0', 
        'Floater': 'N', 'MTG Factor': '1', 'PX Bid': '100', 'PX Mid': '100', 'Int Acc': '0', 'Mtg WAL': '0', 
        'DUR ADJ OAS BID': '', 'YAS_MOD_DUR': '', 'USED DURATION': '', 'Days Acc': '', 'YLD_ytm_BID': '', 'I_SPRD_BID': '', 
        'FLT_SPREAD': '', 'OAS_SPREAD_ASK': '', 'MTG TRANCHE TYP LONG': '', 'MTG PL CPR 1M': '', 'MTG PL CPR 6M': '', 
        ...
        'RTG_FITCH': '', 'RTG_KBRA': '', 'RTG_DBRS': '', 'RTG_Egan_Jones': '', 'DELIVERY_TYP': 'PHYS', "Est'd Asset Class": 'FUNDINT', 
        'CUSIP or ISIN': ' CUSIP', 'MTG_PREV_FACTOR': '', 'MTG_RECORD_DT': '', 'MTG_FACTOR_PAY_DT': '', 'MTG_NXT_PAY_DT_SET_DT': ''},
        {'CUSIP': 'JPM-DYM1', 'SECURITY_TYP': '', 'ISSUER': 'Concord', 'Collat Typ': '', 'Name': 'JPM-DYM1', 
        'Industry Sector': '', 'Issue DT': '8/13/2021', 'Maturity': '8/20/2021', 'Amt Outstanding': '10000000', 'Coupon': '0', 
        'Floater': 'N', 'MTG Factor': '1', 'PX Bid': '100', 'PX Mid': '100', 'Int Acc': '0', 'Mtg WAL': '0', 
        'DUR ADJ OAS BID': '', 'YAS_MOD_DUR': '', 'USED DURATION': '', 'Days Acc': '', 'YLD_ytm_BID': '', 'I_SPRD_BID': '', 
        'FLT_SPREAD': '', 'OAS_SPREAD_ASK': '', 'MTG TRANCHE TYP LONG': '', 'MTG PL CPR 1M': '', 'MTG PL CPR 6M': '', 
        ...
        'RTG_FITCH': '', 'RTG_KBRA': '', 'RTG_DBRS': '', 'RTG_Egan_Jones': '', 'DELIVERY_TYP': 'PHYS', "Est'd Asset Class": 'FUNDINT', 
        'CUSIP or ISIN': ' CUSIP', 'MTG_PREV_FACTOR': '', 'MTG_RECORD_DT': '', 'MTG_FACTOR_PAY_DT': '', 'MTG_NXT_PAY_DT_SET_DT': ''},
        {'CUSIP': 'JPM-4631', 'SECURITY_TYP': '', 'ISSUER': 'Concord', 'Collat Typ': '', 'Name': 'JPM-4631', 
        'Industry Sector': '', 'Issue DT': '8/13/2021', 'Maturity': '8/20/2021', 'Amt Outstanding': '10000000', 'Coupon': '0', 
        'Floater': 'N', 'MTG Factor': '1', 'PX Bid': '100', 'PX Mid': '100', 'Int Acc': '0', 'Mtg WAL': '0', 
        'DUR ADJ OAS BID': '', 'YAS_MOD_DUR': '', 'USED DURATION': '', 'Days Acc': '', 'YLD_ytm_BID': '', 'I_SPRD_BID': '', 
        'FLT_SPREAD': '', 'OAS_SPREAD_ASK': '', 'MTG TRANCHE TYP LONG': '', 'MTG PL CPR 1M': '', 'MTG PL CPR 6M': '', 
        ...
        'RTG_FITCH': '', 'RTG_KBRA': '', 'RTG_DBRS': '', 'RTG_Egan_Jones': '', 'DELIVERY_TYP': 'PHYS', "Est'd Asset Class": 'FUNDINT', 
        'CUSIP or ISIN': ' CUSIP', 'MTG_PREV_FACTOR': '', 'MTG_RECORD_DT': '', 'MTG_FACTOR_PAY_DT': '', 'MTG_NXT_PAY_DT_SET_DT': ''},
        {'CUSIP': 'JPM-PBTA1', 'SECURITY_TYP': '', 'ISSUER': 'Concord', 'Collat Typ': '', 'Name': 'JPM-PBTA1', 
        'Industry Sector': '', 'Issue DT': '8/13/2021', 'Maturity': '8/20/2021', 'Amt Outstanding': '10000000', 'Coupon': '0', 
        'Floater': 'N', 'MTG Factor': '1', 'PX Bid': '100', 'PX Mid': '100', 'Int Acc': '0', 'Mtg WAL': '0', 
        'DUR ADJ OAS BID': '', 'YAS_MOD_DUR': '', 'USED DURATION': '', 'Days Acc': '', 'YLD_ytm_BID': '', 'I_SPRD_BID': '', 
        'FLT_SPREAD': '', 'OAS_SPREAD_ASK': '', 'MTG TRANCHE TYP LONG': '', 'MTG PL CPR 1M': '', 'MTG PL CPR 6M': '', 
        ...
        'RTG_FITCH': '', 'RTG_KBRA': '', 'RTG_DBRS': '', 'RTG_Egan_Jones': '', 'DELIVERY_TYP': 'PHYS', "Est'd Asset Class": 'FUNDINT', 
        'CUSIP or ISIN': ' CUSIP', 'MTG_PREV_FACTOR': '', 'MTG_RECORD_DT': '', 'MTG_FACTOR_PAY_DT': '', 'MTG_NXT_PAY_DT_SET_DT': ''},
        {'CUSIP': 'JPM-SVI1', 'SECURITY_TYP': '', 'ISSUER': 'Concord', 'Collat Typ': '', 'Name': 'JPM-SVI1', 
        'Industry Sector': '', 'Issue DT': '8/13/2021', 'Maturity': '8/20/2021', 'Amt Outstanding': '10000000', 'Coupon': '0', 
        'Floater': 'N', 'MTG Factor': '1', 'PX Bid': '100', 'PX Mid': '100', 'Int Acc': '0', 'Mtg WAL': '0', 
        'DUR ADJ OAS BID': '', 'YAS_MOD_DUR': '', 'USED DURATION': '', 'Days Acc': '', 'YLD_ytm_BID': '', 'I_SPRD_BID': '', 
        'FLT_SPREAD': '', 'OAS_SPREAD_ASK': '', 'MTG TRANCHE TYP LONG': '', 'MTG PL CPR 1M': '', 'MTG PL CPR 6M': '', 
        ...
        'RTG_FITCH': '', 'RTG_KBRA': '', 'RTG_DBRS': '', 'RTG_Egan_Jones': '', 'DELIVERY_TYP': 'PHYS', "Est'd Asset Class": 'FUNDINT', 
        'CUSIP or ISIN': ' CUSIP', 'MTG_PREV_FACTOR': '', 'MTG_RECORD_DT': '', 'MTG_FACTOR_PAY_DT': '', 'MTG_NXT_PAY_DT_SET_DT': ''},
        {'CUSIP': 'JPM-MANT1', 'SECURITY_TYP': '', 'ISSUER': 'Concord', 'Collat Typ': '', 'Name': 'JPM-MANT1', 
        'Industry Sector': '', 'Issue DT': '8/13/2021', 'Maturity': '8/20/2021', 'Amt Outstanding': '10000000', 'Coupon': '0', 
        'Floater': 'N', 'MTG Factor': '1', 'PX Bid': '100', 'PX Mid': '100', 'Int Acc': '0', 'Mtg WAL': '0', 
        'DUR ADJ OAS BID': '', 'YAS_MOD_DUR': '', 'USED DURATION': '', 'Days Acc': '', 'YLD_ytm_BID': '', 'I_SPRD_BID': '', 
        'FLT_SPREAD': '', 'OAS_SPREAD_ASK': '', 'MTG TRANCHE TYP LONG': '', 'MTG PL CPR 1M': '', 'MTG PL CPR 6M': '', 
...
def process_bb_data(raw_df, mktsymbol_map,df_custom_overrides):
    ...
    newrows = []
    hardcoded_usg_rating = ['31424WCR1','3136G4X32','3133ENUV0','3130AGFP5','3130AJP60','31422XLK6','3130AJZ36']
    hardcoded_usgcmo_rating = ['3136ASL95','3137BHE45','3137F8RH8','3137F97B1','3137FLYE8','3137FPZ73','38379Q5M3','38380TXD3','38382CXN6']
    hardcoded_principal_factor_field = ["XS1951177309","XS2606220999","XS2373029664","3134GW5R3","XS2225938831","XS2091648928","XS2373029748","XS2004377136","00908PAB3","00909DAA1","XS2592024009","XS2644211109","XS2644211281","XS2643730695","XS2644210986","XS2592025071"]
    hardcoded_inflation_indexed = ['9128287D6', '912828H45', '912810SM1','912810RF7','912828YL8']
    for cusip, row in df.iterrows(): # no concurrent modification problem
        for col in (cols.split(",") + ['correct_days']):
            try:
                if col == 'Collat Typ':
                    row[col] = (row[col], 'US GOVT')[row['ISSUER'][:5] == 'US TR' or row['ISSUER'][:5] == 'TREAS']
                    ...
                    try:
                        row[col] = datetime.strptime("1900-01-01", "%Y-%m-%d") if not row[col] else datetime.strptime(row[col], "%Y-%m-%d").strftime("%m/%d/%Y")
                    except:
                    ...
                    if cusip in custom_wals:
                        row[col] = depr_wal_since(custom_wals[cusip][1],datetime.today(),custom_wals[cusip][0])
                    else:
                    ...
                    if not row['DUR ADJ OAS BID']:
                       if not row['Mtg WAL']:
                           if not row['YAS_MOD_DUR']:
                                try:
                                    row[col] = ((datetime.strptime(row['Maturity'], "%m/%d/%Y") - datetime.today()).days) / 365.0
                                except:
                                    row[col] = ""
                           else:
                ...
                elif col in {'Name', 'Industry Sector', 'Floater', 'Amt Outstanding','YLD_ytm_BID', 'I_SPRD_BID',
                             'FLT_SPREAD', 'OAS_SPREAD_ASK','MTG TRANCHE TYP LONG', 'MTG PL CPR 1M',
                             'MTG PL CPR 6M', 'MTG_WHLN_GEO1', 'MTG_WHLN_GEO2', 'MTG_WHLN_GEO3'}:
                    row[col] = "" if not row[col] else row[col]
                elif col == 'RATINGS BUCKET':
                    row[col] = lucid_rating(row['RTG_SP'], row['RTG_MOODY'], row['RTG_FITCH'], row['RTG_KBRA'], row['RTG_DBRS'], row['RTG_EGAN_JONES'], row['ISSUER'], row['SECURITY_TYP'])
                    ...
                    if len(cusip) == 12:
                        ...
                        if row['DTC_REGISTERED'] == 'Y' or row['DTC_ELIGIBLE'] == 'Y' or row[col][:3] == 'DTC':
                            row[col] = 'DTC'
                    ...
                    if cusip in mktsymbol_map.keys():
                        ...
                        row[col] = helix_mkt_symbol(row['SECURITY_TYP'], row['ISSUER'], row['Collat Typ'], row['Issue DT'], row['Name'], row['Industry Sector'], row["MTG TRANCHE TYP LONG"], row["Floater"], legacydate)
                elif col == "CUSIP or ISIN":
                    row[col] = " CUSIP" if len(cusip) == 9 else " ISIN"
                elif col == 'correct_days':
                    try:
                        row['TRADE_DT_ACC_INT'] = row['Int Acc'] if not row['TRADE_DT_ACC_INT'] else round(float(row['TRADE_DT_ACC_INT']), 6)
                    except:
                        row['TRADE_DT_ACC_INT'] = row['Int Acc']
                    try:
                        row[col] = int(row['Days Acc'] * row['TRADE_DT_ACC_INT'] / row['Int Acc']) # for finding days accd using today as settle
                    except:
        ...
        row['Days Acc'] = row['correct_days'] # after traversing all cols update the accruals cols
...
def fetch_pni_df(pni_cusips, other_df):
    ...
    for pnic in pni_cusips:
        ...
        if undr in other_df.index: # "underlying" cusip should be in helix already
            outp = outp.append([
                {"CUSIP": pnic, "SECURITY_TYP": "PNI", "ISSUER": other_df.at[undr, "ISSUER"], "Collat Typ": "PNI", "Name": "PNI",
                 "Industry Sector": "Mortgage Securities", "Issue DT": other_df.at[undr, "Issue DT"], "Maturity": other_df.at[undr, "Maturity"],"Amt Outstanding": "1",
                 "Coupon": "0", "Floater": "N", "MTG Factor": "1", "PX Bid": "100", "PX Mid": "100", "Int Acc": "0", "Mtg WAL": "0",
                 "DUR ADJ OAS BID": "0", "YAS_MOD_DUR": "0", "USED DURATION": "0", "Days Acc": "0", "YLD_ytm_BID": "0",
                 "I_SPRD_BID": "0", "FLT_SPREAD": "", "OAS_SPREAD_ASK": "0", "MTG TRANCHE TYP LONG": "PNI",
                 "MTG PL CPR 1M": "0", "MTG PL CPR 6M": "0", "MTG_WHLN_GEO1": "0", "MTG_WHLN_GEO2": "0", "MTG_WHLN_GEO3": "0",
                 "RATINGS BUCKET": "USG", "RTG_SP": "", "RTG_MOODY": "", "RTG_FITCH": "", "RTG_KBRA": "", "RTG_DBRS": "",
                 "RTG_EGAN_JONES": "", "DELIVERY_TYP": "FED", "Est'd Asset Class": other_df.at[undr, "Est'd Asset Class"], "CUSIP or ISIN": " ISIN",
                 'MTG_PREV_FACTOR': '1', 'MTG_RECORD_DT': 'MTG_PREV_FACTOR', 'MTG_FACTOR_PAY_DT': '', 'MTG_NXT_PAY_DT_SET_DT': ''}
            ])
        else:
            print(pnic + " not here")
            outp = outp.append([
                {"CUSIP": pnic, "SECURITY_TYP": "PNI", "ISSUER": "", "Collat Typ": "PNI", "Name": "PNI",
                 "Industry Sector": "Mortgage Securities", "Issue DT": "", "Maturity": "", "Amt Outstanding": "1",
                 "Coupon": "0", "Floater": "N", "MTG Factor": "1", "PX Bid": "100", "PX Mid": "100", "Int Acc": "0", "Mtg WAL": "0",
                 "DUR ADJ OAS BID": "0", "YAS_MOD_DUR": "0", "USED DURATION": "0", "Days Acc": "0", "YLD_ytm_BID": "0",
                 "I_SPRD_BID": "0", "FLT_SPREAD": "", "OAS_SPREAD_ASK": "0", "MTG TRANCHE TYP LONG": "PNI",
                 "MTG PL CPR 1M": "0", "MTG PL CPR 6M": "0", "MTG_WHLN_GEO1": "0", "MTG_WHLN_GEO2": "0", "MTG_WHLN_GEO3": "0",
                 "RATINGS BUCKET": "USG", "RTG_SP": "", "RTG_MOODY": "", "RTG_FITCH": "", "RTG_KBRA": "", "RTG_DBRS": "",
                 "RTG_EGAN_JONES": "", "DELIVERY_TYP": "FED", "Est'd Asset Class": "", "CUSIP or ISIN": " ISIN",'MTG_PREV_FACTOR': '1', 'MTG_RECORD_DT': 'MTG_PREV_FACTOR', 'MTG_FACTOR_PAY_DT': '', 'MTG_NXT_PAY_DT_SET_DT': ''}
            ])
    outp.set_index('CUSIP', inplace=True)
    return outp
...
if __name__ == "__main__":
    ...
    # start bbcomm.exe if haven't already

    # bb_already = ("bbcomm.exe" in [p.name() for p in psutil.process_iter()])
    # bbcomm_path = PureWindowsPath(Path("C:/blp/DAPI/bbcomm.exe"))
    # # Jan 2020, restart bbcomm.exe regardless
    # # if not bb_already:
    # #     subprocess.call([str(bbcomm_path)])
    # os.system('taskkill /f /im bbcomm.exe')
    # subprocess.call([str(bbcomm_path)])

    cusip_set = sys.argv[1] # changed from cusips to filepath to set of cusips so no need to overload cmd line Sept 2020
    ...
    # reverse 
    mktsymbol_map = {k: ('ABSOTHER' if v not in symbol_classifiers.keys() else symbol_classifiers[v]) for k,v in mktsymbol_map.items()}

    if (bond_data_type == "Helix"):
        # include additional cusips to data-fetch
        addl_df = pd.read_excel(Path(PureWindowsPath('S:/Lucid/Data/Bond Data/Non-Collateral Cusips.xlsx')),skiprows=3)
        input_cusips = input_cusips + [x for x in addl_df['Vantage Proxies'] if str(x) != 'nan'] + [x for x in addl_df['Other'] if str(x) != 'nan']
        ...
        bb_cusips = np.setdiff1d(input_cusips, special_cusips) # remove special cusips from bb fetch proc
        bb_cusips = np.setdiff1d(bb_cusips, pni_cusips) # remove pni dummy cusips from bb fetch proc
        raw_df = bb_fetch(bb_cusips)
        df_custom_overrides = bb_fetch_with_overrides(mktsymbol_map)
        procd_df = process_bb_data(raw_df.fillna(""), mktsymbol_map, df_custom_overrides) # crucial to replace the nan's with "" because changed isnull to falsy throughout the processing. did to avoid errors trying to stringify nan
        ...
        df_to_write = process_bb_data(raw_df.fillna(""), mktsymbol_map, pd.DataFrame()) # crucial to replace the nan's with "" because changed isnull to falsy throughout the processing. did to avoid errors trying to stringify nan
    ...
    if (bond_data_type == "Helix"):
     # then do helix and non-collateral cusips (prices fetched daily so bond data is too)
        ...
        savedeststr = "S:/Lucid/Data/Bond Data/Historical/Bond_Data_" + vdate.strftime("%m_%d_%Y") + "_AM.xlsx"
    elif (bond_data_type == "Proxies"):
        # since prices fetched less often here needs to save down separately
        currdest = PureWindowsPath(Path("S:/Lucid/Data/Bond Data/ProxySecurities Bond Data.xlsx"))
        savedeststr = "S:/Lucid/Data/Bond Data/Historical/ProxySecurities_Bond_Data_" + vdate.strftime("%m_%d_%Y") + ".xlsx"
    ...
    # savedeststr = "S:/Lucid/Data/Bond Data/Data Fetching/tmps_Bond_Data_" + vdate.strftime("%m_%d_%Y") + ".xlsx"
    # savedest = PureWindowsPath(Path(savedeststr))
    # wb.save(savedest)
    # wb.close()
    ...
    if (bond_data_type == "Helix"):
        savedeststrPM = "S:/Lucid/Data/Bond Data/Historical/Bond_Data_" + vdate.strftime("%m_%d_%Y") + "_PM.xlsx"
        savedestPM = PureWindowsPath(Path(savedeststrPM))
        wb.save(savedestPM)  # save PM file
    wb.close()

    # df_to_write = fetch_spec_df().append(procd_df,sort=False)
    # df_to_write = df_to_write[(cols.split(","))[1:]]
    # hardwired_adjustments(df_to_write).to_csv(currfile, index_label= 'CUSIP')
    # copyfile(currfile, savefile) # also backup
