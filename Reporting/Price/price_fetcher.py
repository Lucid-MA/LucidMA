...
def idc_fetch(cusips):
    ...
    # test
    idc_req = "GET,(3140H8QR9,13080BAE1, 3136A66B5),(PRC),,, ,Titles=SHORT,DATEFORM=YMD" # confirmed that IDC response format same if doesn't have cusip
...
def fetch_idc(cusips):
    ...
    if response.status_code == 200:
        ...
        # Extract dates from the first line
...
def pd_fetch(cusips):
    ...
    pd_response = subprocess.check_output("py " + py27_pd_path + " " + pddate.strftime("%Y%m%d") + " " + pd_cusip_req)
...
def PM_process():
    ...
    cusip_list = [item[0] for item in records]
    idc_outp = fetch_idc(cusip_list)
