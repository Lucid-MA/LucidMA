...
print_df(df_margin.head(10))
print(f'cash margin pledged is {pledged_cash_margin}')

total_invest = df_cash_balance[(df_cash_balance['Fund'] == fund_name) & (df_cash_balance['Series'] == series_name)]['Projected_Total_Balance'] + df_cash_balance[(df_cash_balance['Fund'] == fund_name) & (df_cash_balance['Series'] == series_name)]['Sweep_Balance']
print(f'Total Invest is {total_invest}')
...
df_result['Wtd Avg Rate'] = np.where(df_result['Investment Amount'] != 0, df_result['WAR'] / df_result['Investment Amount'], None)
df_result['Wtd Avg Spread'] = np.where(df_result['Investment Amount'] != 0, df_result['WAS'] / df_result['Investment Amount'], None)
df_result['Wtd Avg Haircut'] = np.where(df_result['Investment Amount'] != 0, df_result['WAH'] / df_result['Investment Amount'], None)
