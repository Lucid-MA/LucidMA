...
tb_name = "bronze_nexen_cash_sec_table"
    ...
    if (
        ...
        df["data_id"] = df.apply(
            lambda row: hash_string_v2(
                ...
                f"{row['Cash Account Number'] if pd.notnull(row['Account Number']) else ''}"
                f"{row['Transaction Type Name'] if pd.notnull(row['Account Number']) else ''}"
            ),
            axis=1,
        )
        df["File_date"] = date
