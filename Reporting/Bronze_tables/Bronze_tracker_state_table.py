...
if PUBLISH_TO_PROD:
    ...
    processed_files_tracker = (
        bronze_tracker_dir / "Bronze Table Processed Tracker State PROD"
    )
    skipped_files_tracker = (
        bronze_tracker_dir / "Bronze Table files to skip Tracker State PROD"
    )
else:
    engine = engine_staging
    processed_files_tracker = (
        bronze_tracker_dir / "Bronze Table Processed Tracker State"
    )
    skipped_files_tracker = (
        bronze_tracker_dir / "ronze Table files to skip Tracker State"
    )


base_directories = {
    r"S:/Mandates/Operations/Daily Reconciliation/Historical": "TrackerState_",
}


def extract_date_and_indicator(filename):
    """
    This function extracts the date from a filename with the pattern TrackerState_YYYYMMDD.
    ...
    """
    # Regex to match "TrackerState_" followed by a date in "YYYYMMDD" format
    match = re.search(r"TrackerState_(\d{4})(\d{2})(\d{2})", filename)
