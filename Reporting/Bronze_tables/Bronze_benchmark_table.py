...
test_file_path = get_file_path(r"S:/Users/THoang/Benchmark data_Apr 29 2024.xlsx")
    ...
    with engine.connect() as conn:
        try:
            with conn.begin():  # Start a transaction
                ...
                value_placeholders = ", ".join([f":{col.replace(' ', '_').replace('/', '_')}" for col in df.columns])
                update_clause = ", ".join(
                    [
                        f'"{col}"=EXCLUDED."{col}"'
                        for col in df.columns
                        if col != "benchmark_date"  # Assuming "benchmark_date" is unique and used for conflict resolution
                    ]
                )
...
try:
    ...
    benchmark_df = pd.read_excel(test_file_path, sheet_name='Values', header=start_row - 1, usecols=cell_range)

    # Rename the columns
    new_column_names = ["benchmark_date", "1m A1/P1 CP", "3m A1/P1 CP", "1m SOFR", "3m SOFR", "6m SOFR", "1m LIBOR",
                        "3m LIBOR"]
    benchmark_df.columns = new_column_names

    # Convert the 'dates' column to 'YYYY-MM-DD' format
    benchmark_df['benchmark_date'] = benchmark_df['benchmark_date'].dt.strftime('%Y-%m-%d')
