...
fund_descriptions["USG"] = r"""The fund series seeks income generation with 100\% capital preservation and invests solely in US government backed securities (USG) and repurchase agreements secured by USG, subject to the proprietary Lucid Investment Process.
"""

fund_descriptions["PRIME"] = r"""
...
series_descriptions["PRIMEFUND M1"] = r"""
...
series_descriptions["PRIMEFUND C1"] = r"""
...
series_descriptions["PRIMEFUND Q1"] = r"""
...
series_descriptions["PRIMEFUND MIG"] = r"""
...
def accs_since_start(ws, r_col, tstart_col, tend_col, end, start, daycount):
	...
	try:
		...
		return accs_since_start(ws, r_col, tstart_col, tend_col, end - 1, start, daycount) * (1 + r * dt / daycount)
	except:
		print("Breaking accrual recursion: " + ws.title + " " + r_col + str(start) + ":" + r_col + str(end))
		return 1
...
def xmap(varistring, nbars):
	...
	nbars = min(nbars, 16)
	if (len(varistring) > maxchars):
		return [1.7,1.7,1.7,1.40,0.942,0.674,0.558,0.471,0.395,0.35,0.103,0.103,0.255,0.234,0.215,0.198,0.188][nbars]
	else:
...
def return_table_plot(fund_name,prev_pd_return,series_abbrev,r_this_1, r_this_2,comp_a, comp_b, comp_c,r_a,r_b,r_c,s_a_0,s_a_1,s_a_2,s_b_0,s_b_1,s_b_2,s_c_0,s_c_1,s_c_2):
	if fund_name == "USG":
		out = r"""
...
def addl_coll_breakdown(alloc_aa_a,oc_aa_a,alloc_bbb,oc_bbb,alloc_bb,oc_bb):
	if alloc_aa_a == 'n/a':
		...
		return outp.format(alloc_aa_a=alloc_aa_a,oc_aa_a=oc_aa_a,alloc_bbb=alloc_bbb,oc_bbb=oc_bbb,alloc_bb=alloc_bb,oc_bb=oc_bb)


def colltable(inclextra, secured_by, series_abbrev, include_aaa_in_usg_bucket, alloc_aaa,alloc_aa_a,alloc_bbb,alloc_bb,alloc_tbills,alloc_total,oc_total,oc_tbills,oc_bb,oc_bbb,oc_aa_a,oc_aaa):
	if (inclextra):
		out = r"""
		...
		return out.format(
			...
			series_abbrev=series_abbrev,
			usg_aaa_cat= 'US Govt/AAA Repo' if include_aaa_in_usg_bucket else 'US Govt Repo',
			alloc_aaa=alloc_aaa,
...
def plotify(ws, x_col, y_col, start, end):
	...
	for row in range(start, end + 1):
		if ws[x_col + str(row)].value and ws[y_col + str(row)].value:
			outp = outp + "(" + ws[x_col + str(row)].value.strftime("%Y-%m-%d") + "," + str(
				round(100 * ws[y_col + str(row)].value, 3)) + ") "
	return outp


# for performance vs benchmark charts, for series with less than 3 periods data
def snapshot_graph(hspace, graphwidth, graphheight, maxreturn, series_abbrev,comp_a,comp_b, comp_c, this_r, ra, rb, rc):
...
def performance_graph(titleincl,graphhspace,graphwidth,graphheight,fund_name,zero_date,min_return,max_return,graphx,graphbarwidth,return_plot, comp_a_plot,comp_b_plot,series_abbrev,comp_a,comp_b):
	...
	return out.format(
		...
		fund_name = fund_name,
		title = r"""
		title={{Performance vs Benchmark}},""" if titleincl else "",
		zero_date = zero_date,
...
def performance_graph_v2(titleincl,graphhspace,graphwidth,graphheight,fund_name,zero_date,max_return,graphx,graphbarwidth,return_plot, comp_a_plot,comp_b_plot,series_abbrev,comp_a,comp_b):
	if fund_name.upper() == "USG":
		legend_x = 0.3
		legend_y = 1.0
	else:
...
def coupon_plotify(ws, crow, daycount):
	...
	for i in range(startrow, crow + 2):
		...
		addl = addl + r"""\textbf{{""" + ws["F" + str(i)].value.strftime("%m/%d/%y") + r"""}} &"""
		addl = addl + r"""\textbf{{""" + form_as_percent(ws[("N" if daycount == 360 else "O") + str(i)].value, 2) + (r"""{{\tiny (Est'd)}}""" if i == crow + 1 else "") + ("*" if ws["C12"].value == "Monthly1" and ws["E" + str(i)].value.strftime("%m/%d/%y") == "12/30/20" else "") + r"""}} &"""
		addl = addl + benchmark_shorten(ws["U" + str(i)].value) + ("+" if int(10000 * ws["W" + str(i)].value) > 0 else "-") + str(int(abs(ws["X" + str(i)].value))) + " &"
		addl = addl + (r"\$" if i == startrow else r"\hphantom{{\$}}") + "{:,.0f}".format(ws["R" + str(i)].value) + " &"
		addl = addl + (r"\hphantom{{\$}}n/a" if i == crow + 1 else (
				(r"\$" if i == startrow else r"\hphantom{{\$}}") + "{:,.2f}".format(ws["T" + str(i)].value))) + " &"
		addl = addl + ws[("S" if i < crow + 1 else "F") + str(i)].value.strftime("%m/%d/%y") + " &"
		addl = addl + (r"\$" if i == startrow else r"\hphantom{{\$}}") + "{:,.0f}".format(ws["R" + str(i)].value) + " &"
		addl = addl + form_as_percent(ws["AP" + str(i)].value, 1) + (
	...
	for i in range(remaining_rows):
		addl = r"""
		"""
		addl = addl + (r"""& & & & & & & &      \\ """ if i < remaining_rows - 1 else "")
		outp = outp + addl + r"""
...
def month_wordify(months):
	if (months < 12):
		...
		return str(int(months / 12)) + (" Years" if int(months / 12) != 1 else " Year") + (
...
def bps_spread(t, b):
	try:
		...
		return "-" if int(abs(val) * 100) == 0 else (
...
try:
	MASTER_FILEPATH = "S:/Mandates/Funds/Fund Reporting/Master Data_0614.xlsx"
...
for ws in wb.worksheets:
	try:
		if (ws.title != "Mandate Template") and (ws["B2"].value == "Mandate Data"):
			...
			print("For period " + prev_pd_start.strftime("%m/%d") + " - " + this_pd_start.strftime("%m/%d"))
			...
			for col in "CDEFG":
				if bigsheet[col + "6"].value:
					if bigsheet[col + "6"].value.upper() == ws["C9"].value.upper():
						program_size = bigsheet[col + str(overview_row)].value  #post sub/redemp...
						break
			...
			r_a = (
				ws["Y" + str(crow)].value,
				diff_period_rate(ws["F" + str(crow - interval_tuple[0])].value, this_pd_start, daycount,
								 accs_since_start(ws, "Y", "E", "F", crow - interval_tuple[0], 7, daycount),
								 accs_since_start(ws, "Y", "E", "F", crow, 7, daycount)) if (crow - interval_tuple[0] >=7) else "n/a",
				diff_period_rate(ws["F" + str(crow - interval_tuple[1])].value, this_pd_start, daycount,
								 accs_since_start(ws, "Y", "E", "F", crow - interval_tuple[1], 7, daycount),
								 accs_since_start(ws, "Y", "E", "F", crow, 7, daycount)) if (crow - interval_tuple[1] >=7) else "n/a"
			)
			r_b = (
				ws["Z" + str(crow)].value,
				diff_period_rate(ws["F" + str(crow - interval_tuple[0])].value, this_pd_start, daycount,
								 accs_since_start(ws, "Z", "E", "F", crow - interval_tuple[0], 7, daycount),
								 accs_since_start(ws, "Z", "E", "F", crow, 7, daycount)) if (crow - interval_tuple[0] >=7) else "n/a",
				diff_period_rate(ws["F" + str(crow - interval_tuple[1])].value, this_pd_start, daycount,
								 accs_since_start(ws, "Z", "E", "F", crow - interval_tuple[1], 7, daycount),
								 accs_since_start(ws, "Z", "E", "F", crow, 7, daycount))  if (crow - interval_tuple[1] >=7) else "n/a"
			)
			r_c = (
				ws["AA" + str(crow)].value,
				diff_period_rate(ws["F" + str(crow - interval_tuple[0])].value, this_pd_start, daycount,
								 accs_since_start(ws, "AA", "E", "F", crow - interval_tuple[0], 7, daycount),
								 accs_since_start(ws, "AA", "E", "F", crow, 7, daycount)) if (crow - interval_tuple[0] >=7) else "n/a",
				diff_period_rate(ws["F" + str(crow - interval_tuple[1])].value, this_pd_start, daycount,
								 accs_since_start(ws, "AA", "E", "F", crow - interval_tuple[1], 7, daycount),
								 accs_since_start(ws, "AA", "E", "F", crow, 7, daycount)) if (crow - interval_tuple[1] >=7) else "n/a"
			)
			r_this_1 = diff_period_rate(ws["F" + str(crow - interval_tuple[0])].value, this_pd_start, daycount,
										accs_since_start(ws, "N" if daycount == 360 else "O", "E", "F",
														 crow - interval_tuple[0], 7, daycount),
										accs_since_start(ws, "N" if daycount == 360 else "O", "E", "F", crow, 7,
														 daycount)) if (crow - interval_tuple[0] >=7) else "n/a"

			r_this_2 = diff_period_rate(ws["F" + str(crow - interval_tuple[1])].value, this_pd_start, daycount,
										accs_since_start(ws, "N" if daycount == 360 else "O", "E", "F",
														 crow - interval_tuple[1], 7, daycount),
										accs_since_start(ws, "N" if daycount == 360 else "O", "E", "F", crow, 7,
														 daycount)) if (crow - interval_tuple[1] >=7) else "n/a"
			...
			if ws["C5"].value.upper() == "FUND":  # fund (series) report template
				# populate
				if ws["C9"].value == "USG" or ws["C11"].value == "M" or ws["C11"].value == "C1":
					maxreturn = 7
				else:
					maxreturn = 8
				
				if ws["C9"].value == "USG" or ws["C11"].value == "M" or ws["C11"].value == "C1" or ws["C11"].value == "MIG":
					minreturn = 3
				else:
					minreturn = 0
				...
				script = fund_report_template.format(
					...
					toptableextraspace = extraspacefromdesc(fund_descriptions[ws["C9"].value.upper()] + series_descriptions[ws["C6"].value.upper()]),
					series_abbrev=ws["C11"].value + (" (NYP Custom)" if ws["C11"].value == "C1" else ""),
					port_limit = "Quarterly" if "Q" in ws["C11"].value else "Monthly",
					seriesname=ws["C12"].value,
					fund_description= custom_qx_desc if ws["C6"].value.upper() == "PRIMEFUND QX" else fund_descriptions[ws["C9"].value.upper()],
					...
					exp_rat_footnote = exp_rat_footnote(True,hardcoded_exp_cap(ws["C9"].value,ws["C11"].value),round(xl_average(ws,"AD",7,crow), 1)),
					prev_pd_start=prev_pd_start.strftime("%b %d"),
					this_pd_start=this_pd_start.strftime("%b %d"),
					prev_pd_return=form_as_percent(ws[("N" if daycount == 360 else "O") + str(crow)].value, 2),
					prev_pd_benchmark=benchmark_shorten(ws["U" + str(crow)].value),
					prev_pd_outperform=bps_spread(form_as_percent(ws[("N" if daycount == 360 else "O") + str(crow)].value, 2),
										 form_as_percent(r_a[0], 2))[1:],
					this_pd_end=ws["C24"].value.strftime("%b %d"),  # TODO ENSURE matches
					this_pd_est_return=form_as_percent(ws[("N" if daycount == 360 else "O") + str(crow + 1)].value, 2),
					# TODO ensure there
					#this_pd_est_outperform=int(10000 * ws["W" + str(crow + 1)].value), 
					this_pd_est_outperform=ws["X" + str(crow + 1)].value, 
					...
					descstretch = stretches(fund_descriptions[ws["C9"].value.upper()] + series_descriptions[ws["C6"].value.upper()])[0],
					pcompstretch = stretches(fund_descriptions[ws["C9"].value.upper()] + series_descriptions[ws["C6"].value.upper()])[1],
					addl_coll_breakdown = addl_coll_breakdown(
						form_as_percent(ws["AK" + str(crow)].value, 1) if ws["C9"].value != "USG" else "n/a",
						form_as_percent(ws["AR" + str(crow)].value, 1),
						form_as_percent(ws["AL" + str(crow)].value, 1) if ws["C9"].value != "USG" else "n/a",
						form_as_percent(ws["AS" + str(crow)].value, 1),
						form_as_percent(ws["AM" + str(crow)].value, 1) if ws["C9"].value != "USG" else "n/a",
					...
					usg_aaa_cat= 'US Govt Repo' if ws["C9"].value == 'USG' else 'US Govt/AAA Repo',
					...
					return_table_plot=return_table_plot(
						...
						r_b = r_b,
						r_c = r_c,
						s_a_0=bps_spread(form_as_percent(ws[("N" if daycount == 360 else "O") + str(crow)].value, 2),
										 form_as_percent(r_a[0], 2)),  # TODO daycounts for spreads and round to 2 places
						s_a_1=bps_spread(r_this_1, r_a[1]),
						s_a_2=bps_spread(r_this_2, r_a[2]),
						s_b_0=bps_spread(form_as_percent(ws[("N" if daycount == 360 else "O") + str(crow)].value, 2),
										 form_as_percent(r_b[0], 2)),
						s_b_1=bps_spread(r_this_1, r_b[1]),
						s_b_2=bps_spread(r_this_2, r_b[2]),
						s_c_0=bps_spread(form_as_percent(ws[("N" if daycount == 360 else "O") + str(crow)].value, 2),
										 form_as_percent(r_c[0], 2)),
						s_c_1=bps_spread(r_this_1, r_c[1]),
						s_c_2=bps_spread(r_this_2, r_c[2])
					),
					fund_size=wordify(program_size),
					series_size=wordify(ws["G" + str(crow + 1)].value),   # post sub/redemp, TODO temporarily consold hardwired
					...
					performance_graph = performance_graph(
						...
						minreturn,
						maxreturn,
						xmap(fund_descriptions[ws["C9"].value.upper()] + series_descriptions[ws["C6"].value.upper()], ts_row_end-ts_row_start + 1) if ws["C6"].value.upper() != "PRIMEFUND Q1" else 0.065,
						...
						ws["Y6"].value,
						ws["Z6"].value,
					) if (ts_row_end-ts_row_start + 1 > 3) else snapshot_graph(
						-0.8,
						"!",
						"6.676cm",
						0.25+max(round(ws[("N" if daycount == 360 else "O") + str(crow)].value * 100, 2),
						round(r_a[0] * 100, 2),
						round(r_b[0] * 100, 2),
						round(r_c[0] * 100, 2))+0.28,
						ws["C11"].value,
						ws["Y6"].value,
						ws["Z6"].value,
						ws["AA6"].value,
						round(ws[("N" if daycount == 360 else "O") + str(crow)].value *100, 2),
						round(r_a[0] * 100, 2),
						round(r_b[0] * 100, 2),
						round(r_c[0] * 100, 2)
					))
				...
				script = note_report_template.format(
					...
					colltable=colltable(
						not (ws["C9"].value == "USG"),
						secured_by_from(ws["C9"].value, series_from_note(ws["C9"].value, ws["C12"].value)),
					...
					max_return=3,
					return_plot=plotify(ws, 'F', "N" if daycount == 360 else "O", ts_row_start, ts_row_end),
					...
					performance_graph = snapshot_graph(
						...
						"!",
						"6.676cm",
						max(round(ws[("N" if daycount == 360 else "O") + str(crow)].value, 2),
						round(r_a[0] * 100, 2) if r_a[0] is not None else 0,
						round(r_b[0] * 100, 2) if r_b[0] is not None else 0,
						round(r_c[0] * 100, 2) if r_c[0] is not None else 0) + 0.4,
						ws["C11"].value,
						ws["Y6"].value,
						ws["Z6"].value,
					...
					series_size=wordify(ws["G" + str(crow + 1)].value),  # post sub/redemp, TODO temporarily consold hardwired
					lucid_aum=wordify_aum(lucid_aum),
					fund_inception=ws["C18"].value.strftime("%B %d, %Y") if fund_inception_from_name(ws["C9"].value) == "n/a" else fund_inception_from_name(ws["C9"].value),
					cusip=ws["C16"].value,
					...
					rets_disclaimer_if_m1= m1_1230_128_rets_disclaimer if ws["C12"].value == "Monthly1" else ""
				)
			else:
				continue
			...
			cmd_str = "pdflatex -interaction nonstopmode {} {}".format(script_file, pdf_file)
			try:
				try:
