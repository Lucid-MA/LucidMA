...
echo [%date% %time%] Git commit and push started >> "S:\Users\THoang\Tech\BatchLogs\BatchLogs.txt"

:: Commit the changes with the date in the message
git commit -m "Update git EOD %formattedDate%"
if %ERRORLEVEL% neq 0 (
    echo [%date% %time%] Git commit failed >> "S:\Users\THoang\Tech\BatchLogs\BatchLogs.txt"
...
git push
if %ERRORLEVEL% neq 0 (
    echo [%date% %time%] Git push failed >> "S:\Users\THoang\Tech\BatchLogs\BatchLogs.txt"
