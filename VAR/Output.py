...
def Output_Engine(Results: dict,N,DATE): 
    ...
    ledger_list_full={'USG':['Monthly'], 'Prime':['Master','Monthly','Custom1','Quarterly1','MonthlyIG', 'QuarterlyX','Q364','2YIG']}
        ...
        for ledger in ledger_list_full.get(Fund):
            if isinstance(Results.get(('N',Fund,ledger)), pd.DataFrame): 
                ...
                if Fund=='USG':
                    ...
                    wb.save(os.path.join(folder_path,f'Prime Fund_{ledger}_{today_out}.xlsx'))
                wb.close()
                NormalStress=Results.get(('N',Fund,ledger))
                StressedStress=Results.get(('S',Fund,ledger))
                CriticStress=Results.get(('C',Fund,ledger))

                NormalStress = NormalStress.reindex(np.roll(NormalStress.index, shift=1))
                StressedStress=StressedStress.reindex(np.roll(StressedStress.index, shift=1))
                CriticStress=CriticStress.reindex(np.roll(CriticStress.index, shift=1))

                if Fund == 'USG':
                    with pd.ExcelWriter(os.path.join(folder_path,f'USG_Monthly_{today_out}.xlsx'),mode ='a', engine='openpyxl',if_sheet_exists='overlay') as writer:
                        writing(NormalStress, writer, StressedStress, CriticStress)
                else:
                    with pd.ExcelWriter(os.path.join(folder_path,f'Prime Fund_{ledger}_{today_out}.xlsx'),mode ='a', engine='openpyxl',if_sheet_exists='overlay') as writer:
                        writing(NormalStress, writer, StressedStress, CriticStress)



def writing(NormalStress, writer, StressedStress, CriticStress):
    NormalStress.reset_index().to_excel(writer, sheet_name="Results",header=None, startrow=10, startcol=2, index=False)
    StressedStress.reset_index().to_excel(writer, sheet_name="Results",header=None, startrow=10, startcol=17, index=False)
    CriticStress.reset_index().to_excel(writer, sheet_name="Results",header=None, startrow=10, startcol=32, index=False)
...
def Overview(Results,DATE):
    ...
    with pd.ExcelWriter(os.path.join(directory_path, f'VaR_{today_out}_Overview.xlsx'), mode='a', engine='openpyxl', if_sheet_exists='overlay') as writer:
        List_Max={}
        ...
        for (StressRun, Fund, ledger) in Results.keys():
            ...
            df.loc[:, df.columns != 'Spd Diff'].to_excel(writer, sheet_name='Summary', header=None, startrow=start_row + i, startcol=2, index=False)
            ...
            if StressRun=='C':
                ...
                while len(df_max.loc[df_max[VaR_list[param]] != 0]) == 0: 
                    param+=1
                List_Max[(Fund,ledger)]=df_max.loc[df_max[VaR_list[6]] != 0].head(1).index[0]
        
        for (Fund,ledger),counterparty in List_Max.items():
            SingleCounterparty_VaR=SingleCounterpartyVAR(Results,counterparty,Fund, ledger)
            start_row = excel_index.get(Fund, {}).get(ledger)
            if start_row is None:
                continue
