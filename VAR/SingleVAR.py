...
def GettingVantage(message, filename):
    ...
    result = pd.read_excel(
        filename, sheet_name="My Table", usecols="B,G,H,J:AA", skiprows=5
    )
...
def ImportRawData(DATE):
    ...
    with pd.ExcelFile(
        r"S:\Lucid\Investment Committee & Risk\VAR Workspace\proxy.xlsx"
    ) as proxy_file:
        proxy = pd.read_excel(proxy_file, usecols="A,C")
    proxy.rename(columns={"Cusip": "CUSIP proxy"}, inplace=True)

    print("Getting Bond Data")
    BondData = pd.read_excel(
        r"S:\Lucid\Data\Bond Data\Bond Data.xlsx", usecols="A,O", skiprows=4
    )
    BondData.drop([0, 1], inplace=True)

    print("Getting ApprovedCounterpartyDatabase")
    ApprovedCouterpartyDatabase = pd.read_excel(
        r"S:\Lucid\Investment Committee & Risk\Approved Counterparties\Approved CP Database Archive\Approved Counterparty Database_3.22.24.xlsx",
        sheet_name="Credit & Correlation for IMR",
        usecols="B,D,H,J",
        skiprows=5,
    )
    ApprovedCouterpartyDatabase.rename(
        columns={
            ApprovedCouterpartyDatabase.columns[1]: "Grade",
            ApprovedCouterpartyDatabase.columns[2]: "Recovery",
            ApprovedCouterpartyDatabase.columns[3]: "C/B Corr",
        },
        inplace=True,
    )
    ApprovedCouterpartyDatabase["Counterparty"] = ApprovedCouterpartyDatabase[
        "Counterparty"
    ].str.upper()
    ApprovedCouterpartyDatabase["B/B Corr"] = 0.99

    print("Getting AnnualDefaultRates")
    AnnualizedDefaultRates = pd.read_excel(
        r"S:\Lucid\Investment Committee & Risk\Approved Counterparties\Approved CP Database Archive\Approved Counterparty Database_3.22.24.xlsx",
        sheet_name="Credit & Correlation for IMR",
        usecols="S,W",
        skiprows=6,
    ...
    AnnualizedDefaultRates.rename(
        columns={
            AnnualizedDefaultRates.columns[0]: "Grade",
            AnnualizedDefaultRates.columns[1]: "AnnualDefaultRate",
        },
        inplace=True,
    )
    AnnualizedDefaultRates.set_index("Grade", inplace=True)

    ApprovedCouterpartyDatabase = ApprovedCouterpartyDatabase.merge(
        AnnualizedDefaultRates, on="Grade", how="left"
    )

    VantageUSG = GettingVantage(
        "Getting VantageUSG", r"S:\Lucid\Data\Bond Data\Vantage Data USG.xlsm"
    )
    VantageUSG.drop(index=VantageUSG.index[-1], inplace=True)
    VantageUSG.rename(
        columns={
            "Volatility": "Volatility N",
            "Volatility.1": "Volatility S",
            "Volatility.2": "Volatility C",
            "Px Drop": "Px Drop N",
            "Px Drop.1": "Px Drop S",
            "Px Drop.2": "Px Drop C",
            "Liq Period": "Liq Period N",
            "Liq Period.1": "Liq Period S",
            "Liq Period.2": "Liq Period C",
        },
        inplace=True,
    )

    VantagePrime = GettingVantage(
        "Getting VantagePrime", r"S:\Lucid\Data\Bond Data\Vantage Data PRIME.xlsm"
    )
    VantagePrime.rename(
        columns={
            "Volatility": "Volatility N",
    ...
    """
    cursor.execute(query, (today, today, today))
    PRIME_trades_full = pd.DataFrame(
        cursor.fetchall(),
        columns=[
            "Trade ID",
            "Ledger",
...
def VAR_param(Data, StressRun, Fund, ledger, DATE):
    ...
    # TODO: review this:
    # USG_trades["End Date"] = pd.to_datetime(USG_trades["End Date"], errors="coerce")
    # PRIME_trades["End Date"] = pd.to_datetime(PRIME_trades["End Date"], errors="coerce")

    # TODO: test printing
    # print(proxy, BondData, ApprovedCouterpartyDatabase, ConcatVantage, USG_trades[0])
    ...
    def StressedColl(row):
        if row["TradeType"] in ["RepoFree", "Repo"]:
            ...
            test = (
                float(row["Par/Quantity"])
                * (float(row["Current Price"]) + float(row["Int Acc"]))
                / 100
                * (1 - float(row["Error Margin"]))
                * float(row["Issue Factor"])
            )
    ...
    if Fund == "USG":
        VAR_parameters_USG_counterparties = USG_trades.groupby("Counterparty").agg(
            {"End Date": "max"}
        )
        VAR_parameters_USG_counterparties["Today"] = pd.to_datetime(DATE)
        VAR_parameters_USG_counterparties["End Date"] = pd.to_datetime(
            VAR_parameters_USG_counterparties["End Date"], errors="coerce"
        )
        VAR_parameters_USG_counterparties["T"] = (
            VAR_parameters_USG_counterparties["End Date"]
            - VAR_parameters_USG_counterparties["Today"]
        ).dt.days
        VAR_parameters_USG_counterparties = VAR_parameters_USG_counterparties.merge(
            ApprovedCouterpartyDatabase, on="Counterparty", how="left"
        )
        VAR_parameters_USG_counterparties["Default Probability"] = 1 - (
            1
            - VAR_parameters_USG_counterparties["AnnualDefaultRate"] * DefaultRateFactor
        ) ** (VAR_parameters_USG_counterparties["T"] / 365)
        VAR_parameters_counterparties = VAR_parameters_USG_counterparties
    elif Fund == "Prime":
        VAR_parameters_PRIME_counterparties = (
            PRIME_trades.get(ledger).groupby("Counterparty").agg({"End Date": "max"})
        )
        VAR_parameters_PRIME_counterparties["Today"] = pd.to_datetime(DATE)
        VAR_parameters_PRIME_counterparties["End Date"] = pd.to_datetime(
            VAR_parameters_PRIME_counterparties["End Date"], errors="coerce"
        )
        VAR_parameters_PRIME_counterparties["T"] = (
            VAR_parameters_PRIME_counterparties["End Date"]
            - VAR_parameters_PRIME_counterparties["Today"]
        ).dt.days
        VAR_parameters_PRIME_counterparties = VAR_parameters_PRIME_counterparties.merge(
            ApprovedCouterpartyDatabase, on="Counterparty", how="left"
        )
        VAR_parameters_PRIME_counterparties["Default Probability"] = 1 - (
            1
            - VAR_parameters_PRIME_counterparties["AnnualDefaultRate"]
            * DefaultRateFactor
        ) ** (VAR_parameters_PRIME_counterparties["T"] / 365)
        VAR_parameters_PRIME_counterparties[
            "Default Probability"
        ] = VAR_parameters_PRIME_counterparties["Default Probability"].fillna(
            0
        )  # if NaN , then all trades of the ocunterparty are in OPEN (cash management), which means the default Probabilioty is null
    ...
    if Fund == "USG":
        ...
        VAR_parameters_USG_trades = USG_trades.merge(
            ConcatVantage[["CUSIP"]], left_on="BondID", right_on="CUSIP", how="left"
        )
        VAR_parameters_USG_trades = VAR_parameters_USG_trades.merge(
            proxy, how="left", on="Rating"
        )

        # get Vantage CUSIP with proxy if the Bond ID is not found in Vatange CUSIP
        VAR_parameters_USG_trades["Vantage Bond"] = VAR_parameters_USG_trades[
            "CUSIP"
        ].fillna(VAR_parameters_USG_trades["CUSIP proxy"])
        VAR_parameters_USG_trades.drop(columns=["CUSIP", "CUSIP proxy"], inplace=True)

        # Get Vantage data
        VAR_parameters_USG_trades = VAR_parameters_USG_trades.merge(
            ConcatVantage, left_on="Vantage Bond", right_on="CUSIP", how="left"
        )
        VAR_parameters_USG_trades.drop(columns=["CUSIP"], inplace=True)

        # get accrual interest from Bond Data
        VAR_parameters_USG_trades = VAR_parameters_USG_trades.merge(
            BondData, how="left", left_on="BondID", right_on="CUSIP"
        )

        # Creates Error margin from Price Drop taking into account scenario
        VAR_parameters_USG_trades["Error Margin"] = VAR_parameters_USG_trades.apply(
            ErrorMargin, axis=1
        )

        # Creates stressed ccollateral value
        VAR_parameters_USG_trades["Stressed Collateral Value"] = (
            VAR_parameters_USG_trades.apply(StressedColl, axis=1)
        )

        # Volatility based on scenario
        VAR_parameters_USG_trades["Volatility"] = VAR_parameters_USG_trades.apply(
            Volatility, axis=1
        )

        # Liquidation Period (in years) based on scenario
        VAR_parameters_USG_trades["Liquidation Period"] = (
            VAR_parameters_USG_trades.apply(Liquidation, axis=1)
        )

        VAR_parameters_trades = VAR_parameters_USG_trades[
            [
                "Trade ID",
                "End Date",
                "Money",
        ...
        VAR_parameters_PRIME_trades = PRIME_trades.get(ledger).merge(
            ConcatVantage[["CUSIP"]], left_on="BondID", right_on="CUSIP", how="left"
        )
        VAR_parameters_PRIME_trades = VAR_parameters_PRIME_trades.merge(
            proxy, how="left", on="Rating"
        )

        VAR_parameters_PRIME_trades["Vantage Bond"] = VAR_parameters_PRIME_trades[
            "CUSIP"
        ].fillna(VAR_parameters_PRIME_trades["CUSIP proxy"])
        ...
        VAR_parameters_PRIME_trades["Error Margin"] = VAR_parameters_PRIME_trades.apply(
            ErrorMargin, axis=1
        )
        VAR_parameters_PRIME_trades["Stressed Collateral Value"] = (
            VAR_parameters_PRIME_trades.apply(StressedColl, axis=1)
        )
        VAR_parameters_PRIME_trades["Volatility"] = VAR_parameters_PRIME_trades.apply(
            Volatility, axis=1
        )
        VAR_parameters_PRIME_trades["Liquidation Period"] = (
            VAR_parameters_PRIME_trades.apply(Liquidation, axis=1)
        )
        VAR_parameters_trades = VAR_parameters_PRIME_trades[
            [
                "Trade ID",
                "End Date",
                "Money",
    ...
    VAR_parameters_counterparties["T"].fillna(0)  # only margin or on open

    # in case of not existing, to avoid not running the var
    VAR_parameters_counterparties["Grade"].fillna("BB")
    VAR_parameters_counterparties["Recovery"].fillna(0.25)
    VAR_parameters_counterparties["C/B Corr"].fillna(0.6)
    VAR_parameters_counterparties["B/B Corr"].fillna(0.99)
    VAR_parameters_counterparties["AnnualDefaultRate"].fillna(0.02)
    VAR_parameters_counterparties["Default Probability"].fillna(0.0315395005881266)
    ...
    if len(VAR_parameters_counterparties["Counterparty"].unique()) != len(
        VAR_parameters_counterparties
    ):
...
def CholeskyProcess(Data, StressRun, Fund, ledger, DATE):
    ...
    CC_corr = 0.5
    VAR_parameters_counterparties, VAR_parameters_trades = VAR_param(
        Data, StressRun, Fund, ledger, DATE
    )
    ...
    for counterparty in CounterpartyList:
        temp_trade_list = VAR_parameters_trades.loc[
            VAR_parameters_trades["Counterparty"] == counterparty
        ]
        S = len(temp_trade_list.index)  # S number of trades for a single counterparty
        temp_trade_array = np.identity(S + 1)
        BB_Corr = VAR_parameters_counterparties.set_index("Counterparty").loc[
            (counterparty, "B/B Corr")
        ]
        CB_Corr = VAR_parameters_counterparties.set_index("Counterparty").loc[
            (counterparty, "C/B Corr")
        ]
...
def RunVAR(Results, N, Data, StressRun, Fund, ledger, DATE):
    ...
    VAR_parameters_counterparties, VAR_parameters_trades, L_counterparties, L_Trades = (
        CholeskyProcess(Data, StressRun, Fund, ledger, DATE)
    )
    ...
    ProbabilityDefaultList = list(
        VAR_parameters_counterparties.set_index("Counterparty")["Default Probability"]
    )
    RecoveryRateList = list(
        VAR_parameters_counterparties.set_index("Counterparty")["Recovery"]
    )
    Single_Counterparty_S_Trades = (
        VAR_parameters_trades["Counterparty"].value_counts().to_dict()
    )
    Single_Counterparty_VAR_trades_parameters = {
        counterparty: VAR_parameters_trades.loc[
            VAR_parameters_trades["Counterparty"] == counterparty
        ].set_index("Trade ID")
        for counterparty in CounterpartyList
    }
    ...
    for _ in range(N):
        ...
        RV_uncorrelated_temp_Y = np.apply_along_axis(
            ndtri, 0, RV_uncorrelated_Y
        ).tolist()
        ...
        for k, counterparty in enumerate(CounterpartyList):
            ...
            if (
                RV_correlated_Y[k] < ProbabilityDefaultList[k]
            ):  # if default, then calculate rv and lossd for each trade
                temp_trade = Single_Counterparty_VAR_trades_parameters.get(
                    counterparty
                )  # Var trade parameters
    ...
    for key in Portfolio_Loss_Dic:
        ...
        Portfolio_Loss_Dic_Sorted[key] = [
            0 for _ in range(len(unsorted) - len(non_zeros))
        ] + list(
            np.sort(non_zeros)
        )  # type: ignore
    ...
    for counterparty in CounterpartyList:
        counterparty_data = Portfolio_Loss_Dic_Sorted.get(counterparty, [])
        if len(counterparty_data) > 0:
            Portfolio_Loss.loc[counterparty, "Probability of First Default"] = (
                np.count_nonzero(counterparty_data) / N
            )  # type: ignore
        else:
            Portfolio_Loss.loc[counterparty, "Probability of First Default"] = (
                np.nan
    ...
    # Calculate the expected loss, handling NaN
    for counterparty in CounterpartyList:
        counterparty_data = Portfolio_Loss_Dic_Sorted.get(counterparty, [])
        if len(counterparty_data) > 0:
            mean_value = np.mean(counterparty_data)
            if not np.isnan(mean_value):
                Portfolio_Loss.loc[counterparty, "Exp Loss"] = int(mean_value)
            else:
                Portfolio_Loss.loc[counterparty, "Exp Loss"] = np.nan  # Handle NaN case
        else:
            Portfolio_Loss.loc[counterparty, "Exp Loss"] = np.nan  # Handle empty case
    ...
    for param in VARList:
        for counterparty in CounterpartyList:
            counterparty_data = Portfolio_Loss_Dic_Sorted.get(counterparty, [])
            if len(counterparty_data) > 0:
                Portfolio_Loss.loc[counterparty, param] = counterparty_data[
                    int(param * N + 0.5)
                ]
            else:
                Portfolio_Loss.loc[counterparty, param] = np.nan  # Handle empty case

    # Handle "Total FUND"
    total_fund_data = Portfolio_Loss_Dic_Sorted.get("Total FUND", [])
    if len(total_fund_data) > 0:
        Portfolio_Loss.loc["Total FUND", "Probability of First Default"] = (
            np.count_nonzero(total_fund_data) / N
        )
        mean_value = np.mean(total_fund_data)
        if not np.isnan(mean_value):
            Portfolio_Loss.loc["Total FUND", "Exp Loss"] = int(mean_value)
        else:
            Portfolio_Loss.loc["Total FUND", "Exp Loss"] = np.nan
        for param in VARList:
            Portfolio_Loss.loc["Total FUND", param] = total_fund_data[int(param * N)]
    else:
        Portfolio_Loss.loc["Total FUND", "Probability of First Default"] = np.nan
        Portfolio_Loss.loc["Total FUND", "Exp Loss"] = np.nan
        for param in VARList:
            Portfolio_Loss.loc["Total FUND", param] = np.nan

    Portfolio_Loss["Spd Diff"] = ""
    SpdDiff_df = VAR_parameters_trades.groupby("Counterparty").agg(
        {"End Date": "max", "Money": "sum"}
    )
    SpdDiff_df["End Date"] = SpdDiff_df["End Date"].dt.date
    for counterparty in CounterpartyList:
        if isinstance(
            SpdDiff_df.loc[counterparty, "End Date"], pd._libs.tslibs.nattype.NaTType
        ):
            Portfolio_Loss.loc[counterparty, "Spd Diff"] = "OPEN"
        else:
            try:
                Portfolio_Loss.loc[counterparty, "Spd Diff"] = (
                    Portfolio_Loss.loc[counterparty, "Exp Loss"]
                    / SpdDiff_df.loc[counterparty, "Money"]
                    * 365
                    / ((SpdDiff_df.loc[counterparty, "End Date"] - DATE).days)
                )
